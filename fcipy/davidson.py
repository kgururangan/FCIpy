...
def davidson_ci(system, det, e1int, e2int, b, e, e_diag, maxit=200, max_size=30, tolerance=1.0e-08):
    ...
    for niter in range(maxit):
        ...
        if resnorm < tolerance and abs(delta_e) < tolerance:
            ...
            # print("iteration: %d   residual: %.9f   eigenvalue:  %.9f   elapsed time:  %.2f" % (niter, resnorm, e + system.nuclear_repulsion, elapsed_time))
        ...
        elapsed_time = time.time() - t1
        print_dav_iteration(niter, e + system.nuclear_repulsion, resnorm, delta_e, elapsed_time)
        #print("iteration: %d   residual: %.9f   eigenvalue:  %.9f   elapsed time:  %.2f s" % (niter, resnorm, e + system.nuclear_repulsion, elapsed_time))
        curr_size += 1
...
def run_davidson(system, det, e1int, e2int, nroot):
    ...
    for n in range(nroot):
        ...
        print("\n   Energy of initial guess = {:>10.10f}".format(e0))
